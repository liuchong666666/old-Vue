  ...
  export default {
    ...
    created() {
      ...
      let prods = prodTools.getProds();
        ...
        //如果没有商品，return,，，， 如果不执行这步，下面的删完勒，再进入购物车就会报错
        if(Object.keys(prods).length ===0)return;


      //prods{"87":4,"89":10,"95":16}  ele.id->87
      ...
      this.$ajax.get(url)
        .then(res => {
          ...
          this.goodsList.forEach((ele, index) => {
            if (prods[ele.id]) { //数量
              //如果prods里面有这个属性
              //在vue中添加属性，是没办法做到相应的，setter（vue.js官方文档搜$set）

              // ele.num = prods[ele.id], //将每个商品添加num属性，并赋值本地存储的数量
              //   ele.isPicked = true; //将每个商品添加是否勾选属性，

              //添加属性一定要this.$set
              //                       value
              //vm.$set( target, propertyName/index, value )  官方文档--》API-->实例方法/数据--》$set
